...
-- Sin RLS por negocio acá — esta línea de migraciones (producción) todavía
-- no tiene business_id/get_user_business_id() (eso es del track multitenant
-- en docs/database/migrations/multitenant/, no desplegado a producción
-- todavía). Mismo criterio que order_item_flavors, que tampoco tiene RLS
-- propia hoy — el acceso ya está mediado por el backend NestJS (service role).
