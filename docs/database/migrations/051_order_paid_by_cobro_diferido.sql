-- Fase 4 del roadmap de features de la reunión (docs/meet/new-features.md):
-- cobro diferido. Una orden puede existir sin cobrar (payment_method NULL,
-- ya era el caso de las órdenes creadas por el mesero desde la Fase 2) y
-- cobrarse después. No se agrega una columna payment_status separada — se
-- deriva de payment_method IS NULL, evita un estado redundante que se
-- pueda desincronizar.
--
-- paid_by: quién cobró la orden (no necesariamente quién la creó — un
-- pedido de mesero lo cobra un cajero distinto). Para una venta normal de
-- POS (se cobra en el momento de crearla), paid_by = cashier_id.
-- NOTA: sin aplicar todavía en producción (equivalente a 048/049/050).

ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS paid_by uuid REFERENCES public.profiles(id);

-- Backfill: toda orden ya existente tiene payment_method (nunca hubo cobro
-- diferido antes de esta fase), así que quien la creó es quien la cobró.
UPDATE public.orders SET paid_by = cashier_id WHERE payment_method IS NOT NULL AND paid_by IS NULL;

-- Redefinir create_order_atomic: inserta paid_by (null si la orden se crea
-- sin método de pago — ej. mesero — hasta que un cajero la cobre después).
