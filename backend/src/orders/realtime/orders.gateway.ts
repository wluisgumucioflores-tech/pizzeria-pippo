...
import type { OrderCreatedEventResult } from '../types/order-created-event.types';
...
export class OrdersGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket): Promise<void> {
    ...
    try {
      ...
      const requestedBranchId = client.handshake.query.branchId as
        string | undefined;
  ...
  // El payload completo evita que cada cliente conectado a la sucursal
  // (POS, mesero, cocina) tenga que volver a pedir la lista entera de
  // pedidos del día solo para enterarse de uno nuevo.
  emitOrderCreated(branchId: string, payload: OrderCreatedEventResult): void {
