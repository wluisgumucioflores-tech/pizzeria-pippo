  ...
  beforeEach(async () => {
    prisma = {
      ...
      branchStock: {
        ...
        findUnique: jest
          .fn()
          .mockResolvedValue({ branch: { businessId: 'biz1' } }),
        update: jest.fn(),
        create: jest.fn(),
      },
      stockMovement: {
    ...
    it('filtra solo las filas con quantity < min_quantity', async () => {
      prisma.branchStock.findMany.mockResolvedValue([
        ...
        },
        {
          ...
          quantity: decimal(10),
          minQuantity: decimal(5),
          ingredient: { id: 'i2', name: 'Queso', unit: 'kg' },
          branch: { id: 'b1', name: 'Centro' },
        },
      ]);
  ...
  describe('purchase', () => {
    it('suma la cantidad al stock existente y registra el movimiento tipo compra', async () => {
      ...
      await service.purchase(
        { branch_id: 'b1', ingredient_id: 'i1', quantity: 3 },
        admin,
      );
    ...
    it('crea el registro de stock si no existía', async () => {
      ...
      await service.purchase(
        { branch_id: 'b1', ingredient_id: 'i1', quantity: 3, min_quantity: 1 },
        admin,
      );
  ...
  describe('adjust', () => {
    it('calcula la diferencia contra la cantidad existente y la registra como ajuste', async () => {
      ...
      const result = await service.adjust(
        { branch_id: 'b1', ingredient_id: 'i1', real_quantity: 7 },
        admin,
      );
