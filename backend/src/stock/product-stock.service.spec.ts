...
describe('ProductStockService', () => {
  ...
  describe('list', () => {
    it('filtra las filas cuyo producto está inactivo', async () => {
      ...
      expect(prisma.branchProductStock.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { branch: { businessId: 'biz1' }, branchId: 'b1' } }),
      );
    });
  });
    ...
    it('mapea las variantes de reventa activas', async () => {
      ...
      expect(prisma.productVariant.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { businessId: 'biz1', isActive: true, product: { isActive: true, productType: 'resale' } },
        }),
      );
    });
  });
  ...
  describe('purchase', () => {
    it('suma la cantidad si ya existe stock para esa variante en la sucursal', async () => {
      ...
      await service.purchase({ branch_id: 'b1', variant_id: 'v1', quantity: 3 }, admin);
    ...
    it('crea el registro de stock si no existía', async () => {
      ...
      await service.purchase({ branch_id: 'b1', variant_id: 'v1', quantity: 3, min_quantity: 1 }, admin);
    ...
    it('rechaza con 404 si la sucursal pertenece a otro negocio', async () => {
      ...
      await expect(
        service.purchase({ branch_id: 'b1', variant_id: 'v1', quantity: 3 }, admin),
  ...
  describe('adjust', () => {
    it('calcula la diferencia y la registra como ajuste', async () => {
      ...
      const result = await service.adjust({ branch_id: 'b1', variant_id: 'v1', real_quantity: 7 }, admin);
    ...
    it('crea la fila si el producto nunca pasó por bodega/transferencia en esta sucursal', async () => {
      ...
      const result = await service.adjust({ branch_id: 'b1', variant_id: 'v1', real_quantity: 5 }, admin);
  ...
  describe('updateMinQuantity', () => {
    it('actualiza min_quantity por id', async () => {
      prisma.branchProductStock.findUnique.mockResolvedValue({ branch: { businessId: 'biz1' } });
    ...
    it('rechaza con 404 si el registro pertenece a otro negocio', async () => {
      prisma.branchProductStock.findUnique.mockResolvedValue({ branch: { businessId: 'otro-negocio' } });
