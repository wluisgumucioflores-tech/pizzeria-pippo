...
export class StockService {
  ...
  async list(query: ListStockQueryDto, user: CurrentUserPayload): Promise<StockListResult> {
  ...
  async getAlerts(query: ListAlertsQueryDto, user: CurrentUserPayload): Promise<StockRow[]> {
    const where = {
      branch: { businessId: this.resolveBusinessId(user) },
      ...(query.branchId ? { branchId: query.branchId } : {}),
      ingredient: { isActive: true },
  ...
  async getMovements(query: ListMovementsQueryDto, user: CurrentUserPayload): Promise<MovementListResult> {
    const page = query.page ?? 1;
    const pageSize = query.pageSize ?? 10;
    const where = {
      branch: { businessId: this.resolveBusinessId(user) },
  ...
  async purchase(dto: PurchaseStockDto, user: CurrentUserPayload): Promise<void> {
  ...
  async adjust(dto: AdjustStockDto, user: CurrentUserPayload): Promise<{ difference: number }> {
    await this.assertBranchOwnership(dto.branch_id, user);
  ...
  async updateMinQuantity(id: string, minQuantity: number, user: CurrentUserPayload): Promise<void> {
    ...
    const row = await this.prisma.branchStock.findUnique({ where: { id }, select: { branch: { select: { businessId: true } } } });
