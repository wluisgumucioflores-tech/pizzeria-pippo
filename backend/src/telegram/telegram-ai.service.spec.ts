...
describe('TelegramAiService', () => {
  ...
  it('despacha sales_summary a ReportsService.getSales y formatea el resultado', async () => {
    ...
    mockAiClient.complete.mockResolvedValue({
      ...
      content: JSON.stringify({
        ...
        params: { from: '2026-07-14', to: '2026-07-14' },
      }),
    });
    reportsService.getSales.mockResolvedValue({
      total: 150.5,
      count: 3,
      avg: 50.16666,
    });
  ...
  it('despacha stock_alerts a StockService.getAlerts y avisa cuando todo está bien', async () => {
    ...
    mockAiClient.complete.mockResolvedValue({
      type: 'text',
      content: JSON.stringify({ intent: 'stock_alerts', params: {} }),
    });
    ...
    expect(result).toEqual({
      type: 'text',
      content: '✅ Todo el stock está sobre el mínimo.',
    });
  });
      ...
      data: [
        {
          quantity: 5,
          min_quantity: 1,
          ingredients: { name: 'Queso Mozzarella', unit: 'kg' },
          branches: { name: 'Satelite' },
        },
        {
          quantity: 2,
          min_quantity: 1,
  ...
  it('devuelve intent unknown como mensaje genérico', async () => {
    ...
    mockAiClient.complete.mockResolvedValue({
      type: 'text',
      content: JSON.stringify({ intent: 'unknown', params: {} }),
    });

    const result = await service.processMessage('¿qué tiempo hace?');

    expect(result.content).toContain(
      'Solo puedo ayudarte con información del restaurante',
    );
  });
});
