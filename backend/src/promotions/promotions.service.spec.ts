...
describe('PromotionsService', () => {
  ...
  describe('getById', () => {
    it('lanza 404 si no existe', async () => {
      ...
      await expect(service.getById('missing', admin)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('lanza 404 si la promoción pertenece a otro negocio', async () => {
      ...
      await expect(service.getById('p1', admin)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('update', () => {
    it('borra y recrea las reglas (config reemplazable)', async () => {
      ...
      expect(prisma.promotionRule.deleteMany).toHaveBeenCalledWith({
  ...
  describe('patch', () => {
    ...
    it('rechaza con 404 si la promoción pertenece a otro negocio', async () => {
      ...
      await expect(
        service.patch('p1', { active: false }, admin),
      ).rejects.toThrow(NotFoundException);
      expect(prisma.promotion.update).not.toHaveBeenCalled();
    });
  });
